...
function postRequest(apiSection, action, FeedType, AmazonData, MarketplaceIdList,feed, cb) {
    //$('#tabs').tabs('select', 1);
    ...
    var merchantID = urlEncode(AmazonData.SellerId);
    //var marketplaceID = jQuery.trim($('#marketplaceID').attr('value'));
    var awsAccountID = urlEncode(AmazonData.AWSAccessKeyId);
    var authToken = urlEncode(AmazonData.MWSAuthToken);
    var secretKey = jQuery.trim(AmazonData.sharedSecret);

    var appName = 'abooks' //$('#appName').attr('value');
    ...
    _.each(MarketplaceIdList, function (v, i) {
        queryString += "MarketplaceIdList.Id." + i + "=" + v
    })
