...
router.get('/api/books/page', function (req, res) {
    mysql.connection.query("SELECT count(*) as total FROM books;SELECT * FROM books LIMIT "+(req.query.elemsperpage*(req.query.page-1)+1)+","+req.query.total, function(err,records) {
         res.json({data:records[1],pageIndex:req.query.page,pageSize:req.query.elemsperpage,itemsCount:records[0][0].total});
         //debugger
     //res.send('hi')
     })
 });
