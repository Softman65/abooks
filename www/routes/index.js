...
router.get('/api/books/page', function (req, res) {
    ...
    if(req.query.type=='all'){
        ...
        fields = _fields()+',books.vendorListingid,books.price_quantity,iberlibro.price_quantity as price_quantity_Iberlibro,pictures.image as img,(SELECT count(0) from iberlibro where iberlibro.vendorListingid = books.vendorListingid) as C_iberlibro ,(SELECT count(0) from amazon where amazon.vendorListingid = books.vendorListingid) as C_amazon , CONCAT(CONCAT("https://www.bookfinder.com/search/?title=", urlencode(title) ),"&lang=any&st=xl&ac=qr")  as bookfinder '
    }
